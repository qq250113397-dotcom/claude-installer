...
function md5(str) {
  ...
  var binaryStr = new TextEncoder().encode(str);
  var length8 = binaryStr.length;
  var extra = (64 - ((length8 + 8) % 64)) % 64;
  var padded = new Uint8Array(length8 + extra + 8 + 1);
  padded.set(binaryStr);
  padded[length8] = 0x80;
  var view = new DataView(padded.buffer);
  view.setUint32(padded.length - 8, length8 * 8, true);

  var M = [];
  for (var i = 0; i < padded.length; i += 4) {
    M.push(view.getUint32(i, true));
  }
...
export async function onRequestPost({ request }) {
  ...
  try { body = await request.json(); } catch {
    return jsonResp({ ok: false, error: '请求格式错误' }, 400);
  }

  const orderNo = String(body?.order_no || '').trim();
  if (!orderNo) return jsonResp({ ok: false, error: '请先输入订单号' });

  // 测试码
  if (orderNo === 'TEST-NIUGE-2025') {
    return jsonResp({ ok: true, expiry: Date.now() + MEMBER_DAYS * 86400000, desc: '测试会员' });
  }
  ...
  try {
    ...
    return jsonResp({ ok: false, error: '查询失败，请稍后重试' });
  }

  if (mbdData.error) return jsonResp({ ok: false, error: mbdData.error });

  const state = Number(mbdData.state);
  if (state !== 1 && state !== 2) {
    return jsonResp({ ok: false, error: '该订单未完成支付，请确认后重试' });
  }

  return jsonResp({ ok: true, expiry: Date.now() + MEMBER_DAYS * 86400000, desc: mbdData.description || '' });
}
